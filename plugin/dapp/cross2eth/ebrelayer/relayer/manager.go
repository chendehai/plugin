...
// SimLockFromBtc : 模拟从btc锁住brc20，转移到eth
func (manager *Manager) SimLockFromBtc(lock *relayerTypes.LockBRC20, result *interface{}) error {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	if err := manager.checkPermission(); nil != err {
		return err
	}
	chain33Int := manager.chain33Relayer
	err := chain33Int.SimLockFromBtc(lock)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
